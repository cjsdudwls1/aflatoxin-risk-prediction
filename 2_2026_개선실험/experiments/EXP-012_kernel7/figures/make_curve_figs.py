...
ap_pooled = average_precision_score(y_all, p_all)  # 그려진 pooled 곡선의 실제 AP(폴드평균과 별개)
ax.plot(rc, pr, color=OK["blue"], lw=1.5, zorder=3,
        label=f"Pooled (AP = {ap_pooled:.3f})")
...
# ---------- (3) THRESHOLD SWEEP (pooled OOF) ----------
# x=결정 임계값, y=Precision/Recall/F2(pooled OOF 예측 기준). F2 곡선의 봉우리(=운영 임계치) 강조.
th = np.asarray(D[0]["thresholds"], dtype=float)
f2_m = np.empty_like(th); pr_m = np.empty_like(th); rc_m = np.empty_like(th)
for _i, _t in enumerate(th):
    _pred = (p_all >= _t).astype(int)
    _tp = float(((_pred == 1) & (y_all == 1)).sum())
    _fp = float(((_pred == 1) & (y_all == 0)).sum())
    _fn = float(((_pred == 0) & (y_all == 1)).sum())
    _p = _tp / (_tp + _fp) if (_tp + _fp) else 0.0
    _r = _tp / (_tp + _fn) if (_tp + _fn) else 0.0
    pr_m[_i] = _p; rc_m[_i] = _r
    f2_m[_i] = 0.0 if (_p + _r) == 0 else 5 * _p * _r / (4 * _p + _r)
ipk = int(np.nanargmax(f2_m))  # pooled F2 곡선이 최대가 되는 임계값 인덱스
fig, ax = plt.subplots(figsize=(84 * MM, 66 * MM), layout="constrained")
ax.plot(th, f2_m, color=OK["blue"], ls="-", lw=2.0, label="F2", zorder=4)
ax.plot(th, rc_m, color=OK["orange"], ls="--", lw=1.3, label="Recall", zorder=3)
ax.plot(th, pr_m, color=OK["green"], ls="-.", lw=1.3, label="Precision", zorder=3)
ax.axvline(th[ipk], color=OK["vermillion"], ls=":", lw=1.3, zorder=2,
           label=f"F2-optimal th = {th[ipk]:.2f}")
ax.plot(th[ipk], f2_m[ipk], marker="o", ms=5.5, mfc="white",
        mec=OK["vermillion"], mew=1.6, zorder=5)
ax.annotate(f"F2 max = {f2_m[ipk]:.2f}\nat th = {th[ipk]:.2f}",
            xy=(th[ipk], f2_m[ipk]), xytext=(0.44, 0.58),
            arrowprops=dict(arrowstyle="->", lw=0.9, color=OK["black"]),
...
print(f"[check] pooled-curve AP = {ap_pooled:.3f}  (Fig_PR 굵은 선 라벨값; 폴드평균과 다름이 정상)")
